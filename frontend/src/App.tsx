...
export function App() {
  ...
  return <div className={dark ? 'app dark' : 'app'}>
    ...
    <main className="workspace">
      ...
      <section className="canvas-shell"><div className="canvas-toolbar"><span className="canvas-title">画布</span><span className="canvas-hint">拖入图片或视频，开始创作</span><span className="zoom-label">100%</span></div><div className="canvas-area"><div className="grid" />{nodes.length === 0 ? <div className="empty-canvas"><div className="empty-icon">✦</div><h2>开始你的创作</h2><p>从左侧添加节点，或把桌面上的图片、视频拖到这里</p><button className="primary-button" onClick={() => addNode('prompt')}>添加提示词节点</button></div> : <div className="node-layer">{nodes.map((node) => <div className="canvas-node" key={node.id} style={{ left: node.x, top: node.y }}><div className="node-header"><span>{node.title}</span><span>⋮</span></div><div className="node-body">{node.kind === 'prompt' ? <textarea placeholder="写下你的提示词…" /> : <div className="node-placeholder">{node.kind === 'image' ? '图片素材' : node.kind === 'video' ? '视频素材' : '文本节点'}</div>}</div></div>)}</div>}</div></section>
