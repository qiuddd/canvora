...
export const buildServer = () => {
  ...
  app.post('/api/media/thumbnail', async (request, reply) => { try { const body = request.body as { inputPath: string; outputPath: string; atSeconds?: number; ffmpeg?: string }; const result = await createThumbnail(body.inputPath, body.outputPath, body.atSeconds, body.ffmpeg); if (result.exitCode !== 0) return reply.status(400).send({ message: '缩略图生成失败', detail: result.stderr }); return { ok: true }; } catch (error) { return reply.status(400).send({ message: '缩略图生成失败', detail: String(error) }); } });
