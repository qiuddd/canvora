...
- **从输出口拖线到空白处**会弹出按端口类型过滤的功能菜单（只列这一步真能接的节点）。
- **右侧固定一个 AI 对话窗口**，接 DeepSeek；密钥只在后端使用，前端永远不持有。`deepseek-flash` 支持图片输入。
- **画布打组是画布内的操作**：选中两个以上节点后打组，生成可折叠的分组框（不是素材库里的分组）。
- **时间轴片段拖边缘 = 裁剪，不是变速**（2026-09-11 产品所有者明确纠正）：
  - 默认行为是裁剪——从右往左拉就是把后半段裁掉，源时长真的变短（10 秒拉到 7 秒 = 只保留 0~7 秒）。
  - 变速是另一种模式，需要**双击片段**或右键菜单显式切换；切到变速后拖边缘才改变速度且源时长不变。
  - 徽标要能区分两者：裁剪显示「保留 6.2s」，变速显示「4.22x」。
- **画布平移**：中键、Alt+左键，或**按住空格**再拖动。
- **画布上移动节点只能拖标题栏**（2026-09-11 修 bug 后固化）：节点整体**不许**设为 HTML5 `draggable`，否则会和标题栏的指针拖动抢事件，表现成"移动变复制"。素材要拖到时间轴，用标题栏上独立的「⇱ 时间轴」把手。
- **服务商和密钥只在本机后台配置**：前端不出现密钥、也不让用户填服务商/模型 ID；生成节点从后端配置里下拉选，服务端再校验一次能力。
- **生成结果必须回到画布**：结果素材下载登记到当前项目后，在生成节点下方创建结果节点并写 `sourceNodeId`；云端视频任务由前端轮询 `GET /api/tasks` 后补齐。
- **后端自带后台管理页**（`http://127.0.0.1:8787`）：概览 / 服务商 / 项目 / 素材 / 任务五个页签，具备增删改查，并可一键关闭后端。密钥只录入不回显。

## 1.2 与 PRD 的已知偏差（交付说明里要保留）

| 项 | PRD 写法 | 实际实现 | 原因 |
|---|---|---|---|
| 时间轴渲染 | 建议用 Konva | 用 DOM 绝对定位 | 片段在几十个量级 DOM 够用；拖拽与画布共用同一套已验证的指针写法，不必再加依赖 |
| LUT 路径 | 正斜杠即可 | `lut3d=file='C\:/x.cube'` | 实测 `file=C:/x.cube` 会被 ffmpeg 在冒号处切开直接失败，必须冒号转义 + 单引号包裹 |
| 持久化 | SQLite | `canvora-state.json` | 尚未迁移，所有写操作已走串行锁保证不丢更新 |
...
| 版本 | 日期 | 变更 |
|---|---|---|
| v0.0.1 | 2026-09-11 | 接入统一云端生图/生视频适配器、Provider 预置、生成结果落盘和版本发布规范 |
| v0.0.2 | 2026-09-11 | 修素材移动变复制的拖拽冲突；生成节点改为下拉选模型与参数、结果自动回画布；后台状态页升级为可增删改查的管理页 |
| v0.0.3 | 2026-09-13 | 修剪辑播放不可用根因（素材接口支持 Range 分段）、播放改混合时钟帧级同步、片段可拖动/跨轨、贴图轨与调色 UI 落地、图片输入 -loop 1 与贴图时长钳制修导出音画 |
